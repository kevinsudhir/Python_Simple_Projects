...
# # CALCULATOR
# 
# A SIMPLE CALCULATOR THAT CAN PERFORM FEW BASIC AND ADVANCED OPERATIONS
# BASIC OPERATIONS:
# 1. ADDITION
# 2. SUBTRACTION
# 3. MULTIPLICATION
...
# SETTING TITLE OF THE APPLICATION AS Simple Calculator
...
# Creating few user defined functions for the operations involved in the application.
def click(to_print):
	old=e.get()
	e.delete(0, END)
...
# This function clears one character from the rear end of the string.
def bksps():
	current=e.get()
	length=len(current)-1
	e.delete(length, END)

# This function is defined to evaluate the results and prompt the result on the console.
def evaluate():
	ans=e.get()
	ans=eval(ans)
	e.delete(0, END)
...
# Arrangement of buttons for visualition and computation.
